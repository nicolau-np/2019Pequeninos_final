        ...
        <div class="maincontent">
            <div class="maincontentinner">
                <div class="row">
                    <div class="span10">
                        <?php 
                            include_once 'classes/BuscCursoClasse.php';
                            $objBuscCursoClasse = new BuscCursoClasse();
                                $objBuscCursoClasse->setCon($con);
                           $res = $objBuscCursoClasse->buscarCursos();
                           $res2 = $objBuscCursoClasse->buscarClasses();
                        ?>
                        <div class="panel panel-primary">
                            <h4 class="widgettitle"> Escolher curso e classe</h4>
                          
                            <form name="formCondReprovacao" method="POST">
                                <br/>
                                <select name="curso" required="">
                                    <option><?php if(addslashes(htmlspecialchars(isset($_POST['curso'])))): echo addslashes(htmlspecialchars($_POST['curso'])); else: echo "Curso"; endif;?></option>
                                    <?php 
                                     while ($verCur = $res->fetch(PDO::FETCH_OBJ)){
                                    ?>
                                    <option><?php echo $verCur->curso;?></option>
                                     <?php }?>
                                </select>
                                
                                 <select name="classe" required="">
                                    <option><?php if(addslashes(htmlspecialchars(isset($_POST['classe'])))): echo addslashes(htmlspecialchars($_POST['classe'])); else: echo "Classe"; endif;?></option>
                                      <?php 
                                     while ($verCla = $res2->fetch(PDO::FETCH_OBJ)){
                                    ?>
                                    <option><?php echo $verCla->classe;?></option>
                                     <?php }?>
                                </select>
                                
                                <button name="btEnv" class="btn btn-info" type="submit">Buscar</button>
                            </form>
