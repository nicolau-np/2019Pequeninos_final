            ...
            <div class="pageicon"><span class="iconfa-briefcase"></span></div>
             <?php 
                        if(isset($_GET['id_aluno'])){
                            ...
                            $id_dis=addslashes($_GET['id_dis']);
                            $epoca=addslashes($_GET['epoca']);
                            $ano800=addslashes($_GET['ano']);
                            $_SESSION['id_aluno']= addslashes($_GET['id_aluno']);
                            $_SESSION['id_dis']=addslashes($_GET['id_dis']);
                            $_SESSION['epoca']=addslashes($_GET['epoca']);
                            $_SESSION['ano']=addslashes($_GET['ano']);   
                        }
                        else
                        {
                    ...
                    <div class="span10">
                        ...
                        <div class="av">
                           ...
                           $run80->execute();
                             ...
                             if($v71->estado=="ON"): echo"<br/><a href='eliminar_prova.php?id_prova=".$view80->id_prova."&&id_aluno=".$view80->id_aluno."&&id_disciplina=".$id_dis."&&ano=".$ano800."&&epoca=".$epoca."&&curso=".$ver->curso."&&classe=".$ver->classe."&&pagina=lanca7_8.php'>eliminar</a>";
                          endif;
                          echo '</span></td>';
...
if(addslashes(htmlspecialchars(isset($_POST['sav'])))){
   ...
   if($tipo == "Avaliação"):
                                   ...
                                   if($result1 == "yes"):
                                      ...
                                      // $ObjectCalculos->setCon($con);
                                       ...
                                       $result2 = $ObjectCalculos->FazerUpdates($MAC, $CPP, $CT, $id_aluno, $id_dis, $epoca, $ano800);
                                       if($result2 == "yes"):
                                       ...
                                       if($result3 == "yes"):
                                           ...
                                           $ObjectConversao->setComprensao($CF);
                                           $CF_EXTENSO = $ObjectConversao->Converte();
                                           $OBSERVACAO = $ObjectClassDisciplinas->retornaCla6_9($CF);
                                           /**update em cap e cf e observacao*/
                                           $result4 = $ObjectLancFINAIS->updateFinal($CAP, round($CF), $OBSERVACAO, $CF_EXTENSO, $id_aluno, $id_dis, $ano800);
                                           if($result4 == "yes"):
                               ...
                               if($ObjConseFINAIS->getQuantDisciplinas() == $contEstado):
                                   $contEstadoTRANSI = $ObjConseFINAIS->verificarTRANSITAR($ano800, $id_aluno);
                               if($contEstadoTRANSI>= 3):
                                     $obsercacao_finalH="Não Transita";
                               elseif($contEstadoTRANSI<= 2):
...
?>
                            ...
                            <form class="" name="g" method="POST" action="lanca7_8.php">
                         ...
                         <!-- prova global-->
        <?php
        if(addslashes(htmlspecialchars(isset($_POST['sav2'])))){
        $valor = addslashes(htmlspecialchars($_POST['cpe']));
       $ObjectLancFINAIS->setCon($con);
       $ObjectLancFINAIS->buscaFINAISCapCpeCF($id_aluno, $ano800, $id_dis);
       
       $CF = $ObjectCalculos->retornaCF($ObjectLancFINAIS->getCap(), $valor);
        $ObjectConversao->setComprensao($CF);
        $CF_EXTENSO = $ObjectConversao->Converte();
        $OBSERVACAO = $ObjectClassDisciplinas->retornaCla6_9($CF);
        
           /**update em cap e cf e observacao*/
          $result4 = $ObjectLancFINAIS->updateCPE($ObjectLancFINAIS->getCap(), $valor, $CF, $OBSERVACAO, $CF_EXTENSO, $id_aluno, $id_dis, $ano800);
          if($result4 == "yes"):
          echo '<div class = "alert alert-success">Lançamento feito com sucesso!</div>';
          endif;
    ...
    ?>
                        <div class="glo">
                            <form class="" name="go" method="POST" action="lanca7_8.php">
                                <fieldset>
                                    <legend style="font-size: 13px;">Prova Global</legend>
                                    <hr/>
                                <div class="form-inline">
                                    <label>Prova Global:</label>
                                    <input type="text" name="cpe" placeholder="Prova" required=""/>
