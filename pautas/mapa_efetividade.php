    ...
    <div class="rightpanel">
        ...
        <div class="maincontent">
            <div class="maincontentinner">
                <div class="row">
                    <div class="span10">
                        <form name="f89" method="POST" action="mapa_efetividade.php">
                            <div class="form-inline">
                            <label>Mes:</label>
                            <select name="mes" class="" style="width: 100px;">
                            ...
                            <input type="text" name="ano" value="<?php echo date("Y");?>" style="width: 60px;"/>
                            <input name="go13" type="submit" class="btn btn-primary" value="TOTAL DE FALTAS"/>
                            <input name="go14" type="submit" class="btn btn-warning" value="MAPA DE EFECTIVIDADE"/>
                            </div>
                            
                        </form>
                       <hr/>
                        <?php
                        if(addslashes(htmlspecialchars(isset($_POST['go13'])))){
                          $mes111=addslashes(htmlspecialchars($_POST['mes']));
                            $ano111=addslashes(htmlspecialchars($_POST['ano']));
                            echo '<script> window.location.href="efetividade.php?mes='.$mes111.'&&ano='.$ano111.'";</script>';
                            
                      }
                      
                        if(addslashes(htmlspecialchars(isset($_POST['go14'])))){
                          $mes111=addslashes(htmlspecialchars($_POST['mes']));
                            $ano111=addslashes(htmlspecialchars($_POST['ano']));
                            echo '<script> window.location.href="efetividade2.php?mes='.$mes111.'&&ano='.$ano111.'";</script>';
                     ...
                     <div class="span10">
                          <h4 class="widgettitle">Professores</h4>
                ...
                <table id="dyntable" class="table table-bordered responsive">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th class="head0">Nº Agente</th>
                            <th class="head1">Nome Completo</th>
                            <th class="head0">Genero</th>
                            <th class="head1">Categoria</th>
                            <th class="head0">Nº de Faltas</th>
                            <th class="head0">Justificar</th>
                        </tr>
                    </thead>
                    <tbody>
                      <?php 
                   
                    $se="select *from view_professor order by nome asc";
                    $x=$con->prepare($se);
                    $x->execute();
                    while($vr=$x->fetch(PDO::FETCH_OBJ))
                    {
                        ?>
                        ...
                        <tr class="gradeX">
                          <td class="aligncenter"><span class="center">
                           ...
                           <td><?php echo $vr->nAgente;?></td>
                            <td><?php echo $vr->nome;?></td>
                            <td><?php echo $vr->genero;?></td>
                         <td><?php echo $vr->categoria_estudo;?></td>
                         <?php 
                         $ano=date("Y");
                         $estado1="ativo";
                         try {
                           $sql22="select *from view_faltas_prof where id_professor=:id and ano=:ano and estado=:estado";
                         $run22=$con->prepare($sql22);
                         $run22->bindParam(":id",$vr->id_professor,PDO::PARAM_STR);
                         $run22->bindParam(":ano",$ano,PDO::PARAM_STR);
                         $run22->bindParam(":estado",$estado1,PDO::PARAM_STR);
                         $run22->execute();
                         if($run22):
                             $conta=$run22->rowCount();
                         endif;
                         ...
                         <td><a href='just_prof.php?id_prof=<?php echo $vr->id_professor;?>' class="btn btn-primary">Justificar</a></td>
                        </tr>
          <?php }?>
                    </tbody>
                </table> 
