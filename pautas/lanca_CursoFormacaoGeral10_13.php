...
include("classes/classeTrancar.php");
        $anoLectivo=date("Y");
        $contar000=0;
...
echo $dias."-Feira";				
        ...
        <div class="leftmenu">        
            <ul class="nav nav-tabs nav-stacked">
               ...
               <li id="director1" class="dropdown"><a href=""><span class="iconfa-hand-up"></span> Pautas :: <?php if(($contar000>0)&&($_SESSION['tituloMRX']=="Professor Director")): echo $ver000->classe."".$ver000->turma." ".$ver000->turno; endif;?></a>
                ...
                <li class="dropdown" id="directorProfessor1"><a href=""><span class="iconfa-briefcase"></span> Cardeneta</a>
                	<ul>
                     <?php 
                    if($conta_hora2==0):
    ...
    <div class="rightpanel">
        ...
        <ul class="breadcrumbs">
            ...
            <li><a href="#">Caderneta</a> <span class="separator"></span></li>
            <li>Lançamentos</li>
        ...
        <div class="maincontent">
            ...
            //==========   Call class trancar ===============================
                           ...
                           $contarAvaliacao=$run3->rowCount();  
                               ...
                               $media_provasDB=0;  $MAC=0;  $CT=0;  $cf_extenso="?";
                               if($tipo=="Avaliação"):
                                        //so avaliacao
                                        $pglobal=0;
                                        $ava=round($nota);
                                        ...
                                        if($resAva =="OK_ava"){
                                            $MAC = selectTBAvaliacoes($con);
                                            $media_provasDB = selectTB_Provas($con);
                                        }
                                        //============================  Calc Average ================================ 
                                                
                                    if(($media_provasDB=="---") && ($MAC !="---") && ($MAC >0)):
                                           //$CP=(0*0.8)+($IT*0.2);
                                        $CT=($MAC +0)/2;
                                    elseif(($media_provasDB !="---") && ($MAC !="---") && ($MAC >0)):
                                      //$CP=($media_provasDB*0.8)+($IT*0.2);
                                       $CT=($media_provasDB+$MAC)/2;                                 
                                    endif;
                     //================ UPDATING DATA IN tbl_notas TABLE  ====================                   
                                        updateTBNotas($con,$MAC,$CT,$media_provasDB);
                                        $CAP = pickUpCT($con);
                                        salvarProvaGlobal($con,$CAP,$pglobal,$cf_extenso);
                                    echo"<div class='alert alert-success'>Avaliação Feita com sucesso! Nota: ".$ava." MAC: ".$MAC." MPRova: ".$media_provasDB." CT: ".$CT." IDAluno: ".$id_aluno."</div>";
                     ...
                     //================ UPDATING DATA IN tbl_notas TABLE  ====================                   
                                        ...
                                        $CAP = pickUpCT($con);
                                        salvarProvaGlobal($con,$CAP,$pglobal,$cf_extenso);
                                        echo"<div class='alert alert-success'>Prova Feita com sucesso! Nota: ".$prova." MAC: ".$MAC." MPRova: ".$media_provasDB." CT: ".$CT."IDAluno: ".$id_aluno."</div>";
    ...
    function salvarProvaGlobal($con,$CAP,$pglobal,$cf_extenso){
            $obser=0;
            $pglobal=round($pglobal);  $CAP=round($CAP);
                //========================  Calculate average =========================== 
            $cfinal = ($CAP*0.4)+($pglobal*0.6);
            $cfinal=round($cfinal); 
                if($cfinal <=9):
                    $obser="Nao Transita";
                elseif($cfinal >=9.5):
            ...
            $r=$con->prepare($re);
            $r->bindParam(":cap",$CAP,PDO::PARAM_STR);
            $r->bindParam(":cpe",$pglobal,PDO::PARAM_STR);
            $r->bindParam(":cf",$cfinal,PDO::PARAM_STR);
            ...
            if($r->execute()){
                echo"<div class='alert alert-success'>C.Final Feito com sucesso CAP: ".$CAP." CFINAL: ".$cfinal." IDAluno: ".$_SESSION['id_aluno']. " PGlobal: ".$pglobal."</div>";              
            }else {  echo 'erro'; }
    }//End function
    function enabledField($con,$estado){
        $estado1="ON";
         $estado2="OFF";
         $tipo="caderneta";
              ...
              if($run24):
                  echo"<div class='alert alert-success'>Desbloqueio feito com sucesso!</div>";
                           ...
                           ?>
                            ...
                            <form class="" name="g" method="POST" action="lanca_CursoFormacaoGeral10_13.php">
                               
                                 <div class="form-inline">
                                    <label>Tipo</label>
                                    <select name="tipo" class="input-medium">
                                        <option>Avaliação</option>
                                         <?php if($contarProvas<1): echo'<option>Prova</option>';endif;?>
                                    </select>
                                    <label>Nota</label>
...
$se03="update tbl_historico_aluno set aproveitamento=:apro where id_aluno=:id_aluno and anolectivo=:ano";
    ...
    ?>
                        <div class="glo">
                            <form class="" name="go" method="POST" action="lanca_CursoFormacaoGeral10_13.php">
                                <fieldset>
