...
echo $dias."-Feira";				
        ...
        <div class="leftmenu">        
            <ul class="nav nav-tabs nav-stacked">
               ...
               <li id="director1" class="dropdown"><a href=""><span class="iconfa-hand-up"></span> Pautas :: <?php if(($contar000>0)&&($_SESSION['tituloMRX']=="Professor Director")): echo $ver000->classe."".$ver000->turma." ".$ver000->turno; endif;?></a>
                ...
                <li class="dropdown active" id="Admin1"><a href=""><span class="iconfa-th-list"></span> Usuários</a>
                	<ul style="display: block;">
                    	<li class="active"><a href="table-static.php">Novo</a></li>
    ...
    <div class="rightpanel">
        ...
        <ul class="breadcrumbs">
            ...
            <li><a href="table-static.php">Usuários</a> <span class="separator"></span></li>
            <li>Novo</li>
        ...
        <div class="maincontent">
            ...
            <?php
            include("classes/edit_usuario.php");
            if(isset($_GET['id_user'])):
                $id_user89=$_GET['id_user'];
            $id_pessoa89=$_GET['id_pessoa'];
            $foto89=$_GET['foto'];
            $_SESSION['id_user89']=$_GET['id_user'];
            $_SESSION['id_pessoa89']=$_GET['id_pessoa'];
            $_SESSION['foto89']=$_GET['foto'];
            elseif (!isset($_GET['id_usuario'])) :
              $id_user89=$_SESSION['id_user89'];
            $id_pessoa89=$_SESSION['id_pessoa89'];
            $foto89=$_SESSION['foto89'];   
            
            endif;
            $sql3="select *from view_usuario where id_user=:id_user";
            $run3=$con->prepare($sql3);
            $run3->bindParam(":id_user",$id_user89,PDO::PARAM_STR);
            $run3->execute();
            $view=$run3->fetch(PDO::FETCH_OBJ);
            ...
            if(isset($_POST['bt1']))
            {
                ...
                $obj=new edit_usuario;
             
                   /*
                    $obj->cadastro($nome,$palavra1,$palavra2,$estado,$titulo,$arquivo,$arquivo_tmp,$con);
            */
                $resp1=$obj->receber_valores($con,$titulo,$id_user89,$id_pessoa89,$foto89);
                $resp2=$obj->config_perfil($resp1, $arquivo, $arquivo_tmp);
                $resp3=$obj->editar_pessoa($resp2, $nome);
                $obj->editar_user($resp3, $estado);
         
            }
            ?>
                <form class="stdform" action="edita_usuario.php" method="post" enctype="multipart/form-data">
                    	
                        <p>
                            <label>Nome de Usuário:</label>
                            <span class="field"><input type="text" name="nome" class="input-xlarge" placeholder="João Manuel" required="yes" value="<?php echo $view->nome;?>"/></span>
                        </p>
                        
                        
                        <p>
                                <label>Estado</label>
                                <span class="field"><select name="estado" id="selection1" class="input-medium">
                                  <option><?php echo $view->estado;?></option>
                                   <option>ON</option>
                                <option>OFF</option>
                                </select></span>
                            </p>
                            ...
                            <p>
                                <label>Título:</label>
                                <span class="field"><select name="titulo" id="selection3" class="input-medium">
                            <option><?php echo $view->titulo;?></option>
                            <option>Usuário Normal</option>
                            <option>Usuário Normal 2</option>
