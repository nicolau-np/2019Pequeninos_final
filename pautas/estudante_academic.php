...
<title>Estudante Academic</title>
    ...
    <div class="rightpanel">
        ...
        <div class="maincontent">
            <div class="maincontentinner">
                ...
                <form class="form-inline" action="estudante_academic.php" method="GET">
            <label>Curso:</label>
             ...
             <?php  }?>
                   ...
                   <?php  }?>
                        ...
                        <input type="submit" name="pesq" class="btn btn-primary" value="Pesquizar"/>
                        <input type='submit' name='criar_caderneta' value='Criar Caderneta' class='btn btn-default'/>
                ...
                <div class="table-wrapper"><div class="scrollable"><table class="table table-bordered responsive">
                    ...
                    <?php 
                    if(isset($_GET['pesq'])){
                      $curso=  addslashes(htmlspecialchars($_GET['curso']));
                        $classe=addslashes(htmlspecialchars($_GET['classe']));
                        $turma=addslashes(htmlspecialchars($_GET['turma']));
...
$idade=date("Y") - $string[0];
        ...
        $res01 = $objMiniPautas->buscaDisciplinas($curso, $classe, $con);
        echo"<form name='f3' action='estudante_academic.php' method='POST'>"
        . "<select name='discip' class='form-control' required>"
        . "<option value=''>Selecione</value>";
                while($view01 = $res01->fetch(PDO::FETCH_OBJ)){
                    echo '<option>'.$view01->nome.'</option>';
                }
               echo "</select>"
                . "<input type='hidden' name='curso' value='".$curso."'/>==<input type='text' name='classe' value='".$classe."'/>"
                . "==<input type='hidden' name='turma' value='".$turma."'/>==<input type='text' name='turno' value='".$turno."'/>"
                 . "==<input type='text' name='ano' value='".date("Y")."'/>";
               
        echo "<input type='submit' name='lancar_nota' value='Lançar Notas' class='btn btn-success'/>";
        echo "  <input type='submit' name='lancar_faltas' value='Lançar Faltas' class='btn btn-info'/>";
        echo "  <input type='submit' name='verMini_pautas' value='Mini Pautas' class='btn btn-warning'/>";
        echo"</form>";
        else:
        echo"Nenhum aluno encontrado";
                    ...
                    if(addslashes(htmlspecialchars(isset($_GET['criar_caderneta'])))){
                         $curso=  addslashes(htmlspecialchars($_GET['curso']));
                        $classe=addslashes(htmlspecialchars($_GET['classe']));
                        $turma=addslashes(htmlspecialchars($_GET['turma']));
                        $turno=addslashes(htmlspecialchars($_GET['turno']));
                        ...
                        header("location:criar_cardeneta.php?curso=$curso&&classe=$classe&&turma=$turma&&turno=$turno&&ano=$ano"); 
                    }
                        
                    
                    if(addslashes(htmlspecialchars(isset($_POST['lancar_nota'])))):
                        $curso20=  addslashes(htmlspecialchars($_POST['curso']));
                     $classe20=  addslashes(htmlspecialchars($_POST['classe']));
                      $ano20=  addslashes(htmlspecialchars($_POST['ano']));
                      $turno20 = addslashes(htmlspecialchars($_POST['turno']));
                       $turma20=  addslashes(htmlspecialchars($_POST['turma']));
                       $discip=addslashes(htmlspecialchars($_POST['discip']));
                       
                       
        $do0="select *from tbl_curso where curso=:curso";
        $r0=$con->prepare($do0);
        $r0->bindParam(":curso",$curso20,PDO::PARAM_STR);
        $r0->execute();
        $v0=$r0->fetch(PDO::FETCH_OBJ);
        
        $do="select *from tbl_classe where classe=:classe";
        $r=$con->prepare($do);
        $r->bindParam(":classe",$classe20,PDO::PARAM_STR);
        $r->execute();
        $v1=$r->fetch(PDO::FETCH_OBJ);
        
        $do2="select *from tbl_turma where turma=:turma";
        $r2=$con->prepare($do2);
        $r2->bindParam(":turma",$turma20,PDO::PARAM_STR);
        $r2->execute();
        $v2=$r2->fetch(PDO::FETCH_OBJ);
        
        $do3="select *from tbl_turno where turno=:turno";
        $r3=$con->prepare($do3);
        $r3->bindParam(":turno",$turno20,PDO::PARAM_STR);
        $r3->execute();
        $v3=$r3->fetch(PDO::FETCH_OBJ);
        
         $do4="select *from tbl_dis2 where nome=:nome";
        $r4=$con->prepare($do4);
        $r4->bindParam(":nome",$discip,PDO::PARAM_STR);
        $r4->execute();
        $v4=$r4->fetch(PDO::FETCH_OBJ);
          
        header("location:bootstrap.php?curso=$v0->id_curso&&classe=$v1->id_classe&&turma=$v2->id_turma&&turno=$v3->id_turno&&disciplina=$v4->id_di2");
           endif;
                    
                       if(addslashes(htmlspecialchars(isset($_POST['lancar_faltas'])))):
                        $curso20=  addslashes(htmlspecialchars($_POST['curso']));
                     $classe20=  addslashes(htmlspecialchars($_POST['classe']));
                      $ano20=  addslashes(htmlspecialchars($_POST['ano']));
                      $turno20 = addslashes(htmlspecialchars($_POST['turno']));
                       $turma20=  addslashes(htmlspecialchars($_POST['turma']));
                        ...
                        header("location:lanc_falta.php?curso=$curso20&&classe=$classe20&&turma=$turma20&&turno=$turno20&&ano=$ano20&&disciplina=$discip");
                    endif;
                    
                    if(addslashes(htmlspecialchars(isset($_POST['verMini_pautas'])))):
                        $curso20=  addslashes(htmlspecialchars($_POST['curso']));
                     $classe20=  addslashes(htmlspecialchars($_POST['classe']));
                      $ano20=  addslashes(htmlspecialchars($_POST['ano']));
                      $turno20 = addslashes(htmlspecialchars($_POST['turno']));
                       $turma20=  addslashes(htmlspecialchars($_POST['turma']));
                        ...
                        header("location:discussion.php?curso=$curso20&&classe=$classe20&&turma=$turma20&&turno=$turno20&&ano=$ano20&&disciplina=$discip");
                    endif;
