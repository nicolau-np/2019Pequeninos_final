...
<br />
<div class="a"><b>Pagamentos feitos</b></div>
<form name="fom4" method="POST" class="form-inline" action="desfaz_pagamento9.php">
...
$co4="select *from tb_pagamento_provas where id_aluno=:id and ano_lectivo=:an order by id_pagaprova asc";
...
else
{
    while($rt=$result->fetch(PDO::FETCH_OBJ)){
        $sql99="select *from tbl_dis2 where id_di2=:di";
        $run99=$con2->prepare($sql99);
        $run99->bindParam(":di",  $rt->disciplina,  PDO::PARAM_STR);
        $run99->execute();
        $v99=$run99->fetch(PDO::FETCH_OBJ);
?>

<div class="meses_pagos">

	<option value="<?php echo $rt->id_pagaprova;?>"><?php echo $v99->nome;?></option><?php }}?>
</select>
</div>
<br />
<br />
