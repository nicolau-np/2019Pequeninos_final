...
$a=0;
foreach($_POST['tipo_prova'] as $num)
{
    $a++;
}
...
foreach($_POST['tipo_prova'] as $num)
{
    ...
    if($cont>0){
        ...
        $com2="insert into tbl_pagamento_folha (id_aluno,tipo_prova,hora,data_pagamento,mes_pagamento,ano_lectivo,valor_pago,cliente,usuario,n_fatura,estado) values(:id,:tipo,:hora,:data,:mes_pagamento,:ano_lectivo,:valor_pago,:cliente,:usuario,:n_fatura,:estado)";
    ...
    $ru2->bindParam(":valor_pago",$precario,PDO::PARAM_STR);
...
foreach($_POST['tipo_prova'] as $num)
{
    $com="select *from tbl_pagamento_folha where id_aluno=:ida and tipo_prova=:m and ano_lectivo=:an and hora=:hora";
...
$html.="
<tr>
<td>{$bt->tipo_prova}º Trimestre</td>
